...
int ledChannel = 2; // PWM 채널 설정(0~15)
...
void setup() {
  ...
  ledcAttachPin(_LED_R_GPIO_14, ledChannel);  // GPIO Pin, 채널설정
}

void loop() {
  for (int dutyCycle = 0; dutyCycle <= 255; dutyCycle++) {
    // Duty Cycle 값이 0~255까지 순차적으로 증가(LED 밝기 증가)
    ledcWrite(ledChannel,dutyCycle); // ledChannel(PWM 채널0)에 증가하는 Duty Cycle값 설정(write)
    delay(7);
  }
  for (int dutyCycle = 255; dutyCycle >= 0; dutyCycle--) {
    // Duty Cycle 값이 255~0까지 순차적으로 감소(LED 밝기 감소)
    ledcWrite(ledChannel,dutyCycle); // ledChannel(PWM 채널0)에 감소하는 Duty Cycle값 설정(write)
    delay(7);
  }
}
